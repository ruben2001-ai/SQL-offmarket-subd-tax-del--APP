...
- `pipeline_stage` values (shared enum across both tables): `Leads →
  Underwritten → Outreached → Offered → Follow-up → Accepted / Rejected /
...
The full routine this dashboard's data feeds/is fed by — phases, positioning
per campaign, stage machines, scripts, eligibility rules — is documented in
full below.
