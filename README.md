...
`pipeline_stage` column (`DNC / Potential Leads → Outreached → Responded →
Qualified / Not Qualified → Underwriting → Offered → Follow-up → Accepted /
Rejected / Long-term Follow-up` — `Leads` was renamed into `Potential Leads`;
`Follow-up` is a legacy value kept for existing rows; DNC is manually
assigned, not auto-synced from reach method) and RLS restricted to
